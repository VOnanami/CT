...
from model import RED_CNN # the model you need to import
...
from utils import create_train_val_test_dataloaders_from_csv
...
def parse_args():
    ...
    parser.add_argument("--experiment", default="RED_CNN_denoise", help="Experiment name (e.g., for MLflow).")
...
def main(args):
    ...
    print("="*40 + "\n")
    model = RED_CNN(**config["model"]); #the model you need to use in config
    # --- Data Loading ---
    ...
    train_loader, val_loader, test_loader = create_train_val_test_dataloaders_from_csv(
      csv_path=config.data.csv,
      data_prefix=DATA_PREFIX,  # <-- 使用新的 data_prefix 参数
